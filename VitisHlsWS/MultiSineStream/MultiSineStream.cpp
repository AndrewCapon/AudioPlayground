...
#if DEBUG
    void MultiSineStream(const PhaseType phaseInc[cVoices], DataType samples[cVoices * cBlockSamples], uint32_t debug[cBlockSamples])
#else
    void MultiSineStream(const PhaseType phaseInc[cVoices], DataType samples[cVoices * cBlockSamples])
#endif
{
    #pragma HLS INTERFACE mode=s_axilite    port=return    
    #pragma HLS INTERFACE mode=s_axilite    port=phaseInc  
    #pragma HLS INTERFACE mode=axis         port=samples 
#if DEBUG    
