...
// wrapc file define:
#define AUTOTB_TVIN_phaseInc "../tv/cdatafile/c.MultiSineMaster.autotvin_phaseInc.dat"
...
void apatb_MultiSineMaster_hw(void* __xlx_apatb_param_phaseInc, void* __xlx_apatb_param_samples, void* __xlx_apatb_param_debug)
{
  ...
  port0.param = &__xlx_offset_byte_param_phaseInc;
  ...
  port1.param = &__xlx_offset_byte_param_samples;
  ...
  port2.param = { __xlx_apatb_param_phaseInc, __xlx_apatb_param_samples };
  port2.mname = { "phaseInc", "samples" };
  port2.nbytes = { 32, 1536 };
  for (size_t off = 0, i = 0; i < port2.nbytes.size(); ++i) {
    port2.offset.push_back(off);
    off += hls::sim::divide_ceil(port2.nbytes[i], port2.asize);
  }
