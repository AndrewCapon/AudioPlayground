...
entity MultiSine_control_s_axi is
generic (
...
end entity MultiSine_control_s_axi;
...
architecture behave of MultiSine_control_s_axi is
    ...
    -- memory signals
    signal int_phaseInc_address0 : UNSIGNED(2 downto 0);
    signal int_phaseInc_ce0    : STD_LOGIC;
    signal int_phaseInc_q0     : UNSIGNED(31 downto 0);
    signal int_phaseInc_address1 : UNSIGNED(2 downto 0);
    signal int_phaseInc_ce1    : STD_LOGIC;
    signal int_phaseInc_we1    : STD_LOGIC;
    signal int_phaseInc_be1    : UNSIGNED(3 downto 0);
    signal int_phaseInc_d1     : UNSIGNED(31 downto 0);
    ...
    signal int_samples_address1 : UNSIGNED(8 downto 0);
    ...
    component MultiSine_control_s_axi_ram is
        generic (
    ...
    end component MultiSine_control_s_axi_ram;
...
-- ----------------------- Instantiation------------------
-- int_phaseInc
int_phaseInc : MultiSine_control_s_axi_ram
generic map (
     MEM_STYLE => "auto",
     MEM_TYPE  => "2P",
     BYTES     => 4,
...
port map (
     clk0      => ACLK,
     address0  => int_phaseInc_address0,
     ce0       => int_phaseInc_ce0,
     we0       => (others=>'0'),
     d0        => (others=>'0'),
     q0        => int_phaseInc_q0,
     clk1      => ACLK,
     address1  => int_phaseInc_address1,
     ce1       => int_phaseInc_ce1,
     we1       => int_phaseInc_be1,
     d1        => int_phaseInc_d1,
     q1        => int_phaseInc_q1);
-- int_debug
int_debug : MultiSine_control_s_axi_ram
generic map (
     MEM_STYLE => "auto",
     MEM_TYPE  => "T2P",
     BYTES     => 4,
...
port map (
     ...
     q1        => int_debug_q1);
-- int_samples
int_samples : MultiSine_control_s_axi_ram
generic map (
     MEM_STYLE => "auto",
     MEM_TYPE  => "S2P",
     BYTES     => 4,
...
-- ----------------------- Memory logic ------------------
    -- phaseInc
    int_phaseInc_address0 <= UNSIGNED(phaseInc_address0);
    int_phaseInc_ce0     <= phaseInc_ce0;
    phaseInc_q0          <= STD_LOGIC_VECTOR(RESIZE(int_phaseInc_q0, 32));
    int_phaseInc_address1 <= raddr(4 downto 2) when ar_hs = '1' else waddr(4 downto 2);
    int_phaseInc_ce1     <= '1' when ar_hs = '1' or (int_phaseInc_write = '1' and WVALID  = '1') else '0';
    ...
    int_samples_address1 <= raddr(10 downto 2) when ar_hs = '1' else waddr(10 downto 2);
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                ...
                if (ar_hs = '1' and raddr >= ADDR_PHASEINC_BASE and raddr <= ADDR_PHASEINC_HIGH) then
                    int_phaseInc_read <= '1';
                else
                    int_phaseInc_read <= '0';
                end if;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                ...
                if (aw_hs = '1' and UNSIGNED(AWADDR(ADDR_BITS-1 downto 0)) >= ADDR_PHASEINC_BASE and UNSIGNED(AWADDR(ADDR_BITS-1 downto 0)) <= ADDR_PHASEINC_HIGH) then
...
entity MultiSine_control_s_axi_ram is
    generic (
        MEM_STYLE : STRING :="auto";
        MEM_TYPE  : STRING :="S2P";
        BYTES   : INTEGER :=4;
...
end entity MultiSine_control_s_axi_ram;

architecture behave of MultiSine_control_s_axi_ram is
